...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 MEMEIUM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on memeiumcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
        ...
        "Vote on a governance object by smartnode configured in memeium.conf\n"
        "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("memeium.conf", statusObj));
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by smartnode configured in memeium.conf\n"
#ifdef ENABLE_WALLET
...
static const CRPCCommand commands[] =
    {
        ...
        //  --------------------- ------------------------  -----------------------  ------ ----------
        /* Memeium features */
        {"memeium", "getgovernanceinfo", &getgovernanceinfo, true, {}},
        {"memeium", "getsuperblockbudget", &getsuperblockbudget, true, {"index"}},
        {"memeium", "gobject", &gobject, true, {}},
        {"memeium", "voteraw", &voteraw, true, {}},
