...
void static MemeiumMiner(const CChainParams& chainparams)
{
    LogPrintf("MemeiumMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("memeium-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("MemeiumMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("MemeiumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("MemeiumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("MemeiumMiner -- terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("MemeiumMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateMemeiums(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(boost::bind(&MemeiumMiner, boost::cref(chainparams)));
    }
