...
QVariant MyRestrictedAssetsTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (orientation == Qt::Horizontal) {
        if (role == Qt::DisplayRole) {
            ...
            case AssetName:
                return tr("The asset (or MMM) removed or added to balance.");
            }
        }
    }
    return QVariant();
...
static void NotifyTransactionChanged(MyRestrictedAssetsTableModel* ttm, CWallet* wallet, const std::string& address, const std::string& asset_name, int type, uint32_t date)
{
