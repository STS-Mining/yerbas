...
PACKAGE_VERSION=$($SRC_PATH/src/memeiumd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/memeiumd $DEB_BIN
cp $SRC_PATH/src/memeium-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/memeium-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/memeiumd.1 $DEB_MAN
cp $SRC_DOC/man/memeium-cli.1 $DEB_MAN
cp $SRC_DOC/man/memeium-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/memeiumd.bash-completion $DEB_CMP/memeiumd
cp $SRC_PATH/contrib/memeium-cli.bash-completion $DEB_CMP/memeium-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/memeiumd.1
gzip --best -n $DEB_MAN/memeium-cli.1
gzip --best -n $DEB_MAN/memeium-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/memeiumd $DEB_BIN/memeium-cli
