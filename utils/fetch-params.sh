...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/MemeiumParams"
else
    PARAMS_DIR="$HOME/.memeium-params"
fi
...
DOWNLOAD_URL="https://github.com/The-Memeium-Endeavor/memeium/releases/tag/v3.1.4.20"
...
# fetch methods can be disabled with MMM_DISABLE_SOMETHING=1
MMM_DISABLE_WGET="${MMM_DISABLE_WGET:-}"
#MMM_DISABLE_IPFS="${MMM_DISABLE_IPFS:-}"
MMM_DISABLE_CURL="${MMM_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$MMM_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$MMM_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$MMM_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Memeium parameters!
...
This script will fetch the Memeium SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Memeium SNARK parameters. Note that it is
