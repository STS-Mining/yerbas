...
// which are part of the Memeium repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="memeium-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /memeium-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "memeium-memeium/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from memeium-memeium/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'memeium-memeium/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from memeium-memeium/develop"
            }
          }

          if (hasCache) {
            sh "cd /memeium-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /memeium-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /memeium-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /memeium-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /memeium-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /memeium-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /memeium-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
