...
- [Development guidelines](#development-guidelines)
  - [General Memeium Core](#general-memeium-core)
  - [Wallet](#wallet)
  - [General C++](#general-c)
...
Memeium Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/The-Memeium-Endeavor/memeium/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/memeiumd -printtoconsole
```
...
Memeium Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Memeium Core, where everyone uses
...
pay attention to for reviewers of Memeium Core code.

## General Memeium Core
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - _Rationale_: Memeium Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Memeium Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - _Rationale_: This is impossible to use with `memeium-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - _Rationale_: `memeium-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - _Rationale_: In previous versions of Memeium Core, the wallet was always
